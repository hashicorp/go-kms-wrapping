...
create trigger immutable_columns_kms_version
before update on kms_version
for each row 
  when 
...
when 
  ...
  begin
    update kms_version set update_time = datetime('now','localtime') where rowid == new.rowid;
  end;
