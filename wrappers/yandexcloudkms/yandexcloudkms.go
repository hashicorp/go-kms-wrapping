...
func (k *Wrapper) setClient(client kms.SymmetricCryptoServiceClient) error {
	...
	out, err := k.Encrypt(context.Background(), []byte("go-kms-wrapping-test"), nil)
	if err != nil {
		return fmt.Errorf(
			"failed to encrypt with Yandex.Cloud KMS key - ensure the key exists and permission to encrypt the key is granted: %w", err)
	}
	if _, err := k.Decrypt(context.Background(), out, nil); err != nil {
		return fmt.Errorf(
			"failed to decrypt with Yandex.Cloud KMS key - ensure the key exists and permission to decrypt the key is granted: %w", err)
	}
