...
func newTransitClient(logger hclog.Logger, opts *options) (*TransitClient, *wrapping.WrapperConfig, error) {
	...
	switch {
	case os.Getenv(EnvTransitWrapperMountPath) != "" && !opts.withDisallowEnvVars:
		mountPath = os.Getenv(EnvTransitWrapperMountPath)
	case os.Getenv(EnvVaultTransitSealMountPath) != "" && !opts.withDisallowEnvVars:
	...
	switch {
	case os.Getenv(EnvTransitWrapperKeyName) != "" && !opts.withDisallowEnvVars:
		keyName = os.Getenv(EnvTransitWrapperKeyName)
	case os.Getenv(EnvVaultTransitSealKeyName) != "" && !opts.withDisallowEnvVars:
	...
	switch {
	case os.Getenv(EnvTransitWrapperDisableRenewal) != "" && !opts.withDisallowEnvVars:
		disableRenewalRaw = os.Getenv(EnvTransitWrapperDisableRenewal)
	case os.Getenv(EnvVaultTransitSealDisableRenewal) != "" && !opts.withDisallowEnvVars:
	...
	switch {
	case os.Getenv("VAULT_NAMESPACE") != "" && !opts.withDisallowEnvVars:
